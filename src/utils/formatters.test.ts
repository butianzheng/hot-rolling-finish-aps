...
describe('formatters', () => {
  ...
  describe('formatCapacity', () => {
    it('应该格式化产能（保留1位小数）', () => {
      expect(formatCapacity(1234.567)).toBe('1234.6');
      expect(formatCapacity(100.12)).toBe('100.1');
    });

    it('应该处理整数产能', () => {
      expect(formatCapacity(1000)).toBe('1000.0');
    });

    it('应该处理小数产能', () => {
      expect(formatCapacity(0.5)).toBe('0.5');
      expect(formatCapacity(99.99)).toBe('100.0');
    });
    ...
    it('应该处理负数产能', () => {
      expect(formatCapacity(-50.5)).toBe('-50.5');
    });
  });
});

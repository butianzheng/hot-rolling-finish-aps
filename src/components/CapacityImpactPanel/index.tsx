    ...
    >
      ...
      <Space direction="vertical" style={{ width: '100%' }} size={8}>
        <Space size={16}>
          ...
          <Statistic
            title="变化量"
            value={Math.abs(capacityDelta).toFixed(3)}
            suffix="t"
        ...
        {materialDetails.length > 0 && (
          <div>
            <Text type="secondary" style={{ fontSize: 12 }}>
              选中 {materialDetails.length} 件物料，合计 {affectedWeight.toFixed(3)}t
            </Text>
            <div style={{ marginTop: 4 }}>
              {materialDetails.slice(0, 3).map((m, idx) => (
                <Tooltip
                  key={idx}
                  title={`${m.material_id} - ${m.weight_t.toFixed(3)}t - ${m.urgent_level} - ${m.status}`}
                >
                  <Tag
                    style={{
                      fontSize: 11,
                  ...
                  >
                    {m.material_id} ({m.weight_t.toFixed(3)}t)
                  </Tag>
                </Tooltip>
              ))}

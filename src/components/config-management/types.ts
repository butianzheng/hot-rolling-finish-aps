...
export const configKeyLabels: Record<string, string> = {
  ...
  // 紧急等级阈值配置
  urgent_n1_days: '一级紧急阈值',
  urgent_n2_days: '二级紧急阈值',
  ...
  // 堵塞评分 堵塞评分配置
  d4_capacity_hard_threshold: '堵塞评分 产能硬阈值',
  d4_capacity_full_threshold: '堵塞评分 产能满载阈值',
  d4_structure_dev_threshold: '堵塞评分 结构偏差阈值',
  d4_structure_dev_full_multiplier: '堵塞评分 结构满载倍数',
  d4_structure_small_category_threshold: '堵塞评分 小类忽略阈值',
  d4_structure_violation_full_count: '堵塞评分 结构违规满载数',
  d4_bottleneck_low_threshold: '堵塞评分 等级-低阈值',
  d4_bottleneck_medium_threshold: '堵塞评分 等级-中阈值',
  d4_bottleneck_high_threshold: '堵塞评分 等级-高阈值',
  d4_bottleneck_critical_threshold: '堵塞评分 等级-严重阈值',
  ...
  path_width_tolerance_mm: '路径宽度容差（毫米）',
  path_thickness_tolerance_mm: '路径厚度容差（毫米）',
...
export const configDescriptions: Record<string, string> = {
  // 季节与适温配置
  season_mode: '季节模式（自动判断或手动指定）',
  winter_months: '冬季月份定义（逗号分隔，如：11,12,1,2,3）',
  manual_season: '手动指定季节（冬季或夏季）',
  min_temp_days_winter: '冬季最小适温天数（默认3天）',
  min_temp_days_summer: '夏季最小适温天数（默认4天）',

  // 机组代码配置
  standard_finishing_machines: '标准精整机组代码列表（逗号分隔，如：机组甲,机组乙,机组丙）',
  machine_offset_days: '非标机组出料偏移天数（默认4天）',

  // 紧急等级阈值配置
  urgent_n1_days: '一级紧急天数阈值（临期关注，可选：2/3/5天，默认2天）',
  urgent_n2_days: '二级紧急天数阈值（临期提示，可选：7/10/14天，默认7天）',
  ...
  // 结构校正配置
  target_ratio: '目标钢种配比（数据格式，如：{"钢种甲":0.3,"钢种乙":0.5}，空对象{}表示不启用）',
  deviation_threshold: '结构偏差阈值（允许的目标配比偏差，默认0.1即10%）',
  rhythm_deviation_threshold: '每日生产节奏偏差阈值（用于节奏监控的最大偏差阈值，默认0.1即10%）',

  // 堵塞评分 堵塞评分配置
  d4_capacity_hard_threshold: '产能硬阈值（已用/上限）。低于该值不计堵塞，仅提示。默认0.95。',
  d4_capacity_full_threshold: '产能满载阈值（已用/上限）。用于将>硬阈值的利用率线性映射为0~1严重度，默认1.0。',
  ...
  d4_bottleneck_low_threshold: '堵塞等级阈值-低（严重度0~1）。低于该值为“无”，默认0.30。',
  d4_bottleneck_medium_threshold: '堵塞等级阈值-中（严重度0~1），默认0.60。',
  d4_bottleneck_high_threshold: '堵塞等级阈值-高（严重度0~1），默认0.90。高与严重视为堵塞。',
  d4_bottleneck_critical_threshold: '堵塞等级阈值-严重（严重度0~1），默认0.95。',
  ...
  // 宽厚路径规则（v0.6）
  path_rule_enabled: '是否启用“由宽到窄、由厚到薄”的路径约束（是/否）',
  path_width_tolerance_mm: '宽度容差（单位：毫米）。候选宽度允许小于锚点宽度的最大差值，超过则判定违规。',
  path_thickness_tolerance_mm: '厚度容差（单位：毫米）。候选厚度允许小于锚点厚度的最大差值，超过则判定违规。',
  path_override_allowed_urgency_levels:
    '允许人工确认突破的紧急等级列表（逗号分隔，如：二级,三级）。不在列表内的违规将被硬拦截。',
  seed_s2_percentile:

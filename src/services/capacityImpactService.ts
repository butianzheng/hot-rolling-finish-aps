...
export function predictRemovalImpact(
  ...
  if (affectedWeight === 0) {
    ...
    message = `✓ 移除后产能将改善至 ${predictedCapacity.toFixed(3)}t（目标:${timeline.targetCapacity.toFixed(3)}t）`;
  } else if (improves) {
    message = `✓ 产能利用率降低 ${Math.abs(utilizationChangePercent).toFixed(1)}% 到 ${predictedUtilization.toFixed(1)}%`;
  } else if (exceedsLimitAfter) {
    message = `✗ 移除后仍超限制产能（${predictedCapacity.toFixed(2)}/${timeline.limitCapacity.toFixed(3)}t）`;
  } else if (exceedsTargetAfter) {
    message = `⚠️  移除后仍超目标产能（${predictedCapacity.toFixed(2)}/${timeline.targetCapacity.toFixed(3)}t）`;
  } else {
    message = `产能将变化 ${capacityDelta.toFixed(3)}t`;
  }
...
export function predictAdditionImpact(
  ...
  let message = '';
  if (exceedsLimitAfter) {
    message = `✗ 添加后将超限制产能（${predictedCapacity.toFixed(2)}/${timeline.limitCapacity.toFixed(3)}t）`;
  } else if (exceedsTargetAfter) {
    message = `⚠️  添加后将超目标产能（${predictedCapacity.toFixed(2)}/${timeline.targetCapacity.toFixed(3)}t）`;
